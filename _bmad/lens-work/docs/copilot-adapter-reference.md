...
All `.github/` adapter files are **generated by the CI/CD pipeline** (`promote-to-release.yml`) via the `_module-installer/installer.js`. Source of truth for these files is the installer logic in `bmad.lens.src/_bmad/lens-work/_module-installer/`.
